...
class Keys {
    ...
    replaceSymbol(str) {
        ...
        let lenLeft = !!str.match(/\(/g) ? str.match(/\(/g).length : 0;
        let lenRight = !!str.match(/\)/g) ? str.match(/\)/g).length : 0;
        if (lenLeft !== lenRight) {
    ...
    format(str, sym, rep, isSingle = true) {
        let re1;
        let left;
        let right;
        let lenLeft;
...
// 获取操作输入显示节点；
const showInput = operaShow.children[0];
// 获取结果显示节点；
const showResult = operaShow.children[1];
// 获取历史记录节点：
...
keyboard.addEventListener('click', (ev) => {
    ...
    if (keys.nums.indexOf(key) !== -1) {
        ...
        if ((input + key).match(/÷0|[1-9Π]\(|\)[1-9Π]|\(\)|\dΠ|Π\d|!\d|\.[\+\-x÷%=]|[\+\-x÷%=]\./))
            return false;
        if (key === "\)") {
